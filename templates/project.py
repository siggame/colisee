...
parser = ArgumentParser(prog="colisee-project", description="Create new projects from project templates.")
parser.add_argument("name", help="Name for the new project")
parser.add_argument("type", choices=["library", "service"], help="Type of the new project")
...
contents = [join("common", file) for file in listdir("common")] + [join(args.type, file) for file in listdir(args.type)]
...
for file in contents:
    ...
    if is_template == None:
        ...
        with open(file, 'r') as template, open(join(project, basename(is_template.group(1))), 'w') as asset:
